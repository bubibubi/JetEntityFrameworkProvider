...
namespace JetEntityFrameworkProvider.Utilities
{
    ...
    class FunctionStubFileWriter
    {
        ...
        public StringWriter GenerateCode(string namespaceString, string className, string attributeNamespace, Boolean pascalCaseFunctionNames)
        {
            ...
            foreach (EdmFunction function in _functions)
            {
                ...
                foreach (FunctionParameter parameter in functionParameters)
                {
                    ...
                    if (parameter.TypeUsage.EdmType.GetType() == typeof(CollectionType))
                    {
                        isAggregateFunction = true;
                        if (parameterNameToUse.ToLowerInvariant().Contains("string"))
                            hasStringInParameterName = true;

                        CollectionType collectionType = (CollectionType)parameter.TypeUsage.EdmType;
                        ...
                        if (functionParameters.Count == 1)
                            functionSignatureString.Append("this ");
        ...
        private void GenerateUsingStatements(StringWriter newCode)
        {
            ...
            newCode.WriteLine("using System.Data.Entity;");
            newCode.WriteLine("using System.Data.Entity.Core.Objects;");
        ...
        private void GenerateFunctionHeader(StringWriter newCode, String attributeNamespace, String functionName)
        {
            ...
            newCode.WriteLine("/// </summary>");
            AppendSpaces(newCode, 8);
            newCode.WriteLine("[DbFunctionAttribute(\"" + attributeNamespace + "\", \"" + functionName + "\")]");
        }
        ...
        public void GenerateBetween(StringWriter newCode)
        {
            newCode.WriteLine("        /// <summary>");
            newCode.WriteLine("        /// Use the extension method to implement the Between operation");
            newCode.WriteLine("        /// </summary>");
            newCode.WriteLine("        /// <typeparam name=\"TSource\">Type of the entity</typeparam>");
            newCode.WriteLine("        /// <typeparam name=\"TKey\">Type of the return value</typeparam>");
            newCode.WriteLine("        /// <param name=\"source\">The entity used to apply the method</param>");
            newCode.WriteLine("        /// <param name=\"keySelector\">The lambda expression used to get the return value</param>");
            newCode.WriteLine("        /// <param name=\"low\">Low boundary of the return value</param>");
            newCode.WriteLine("        /// <param name=\"high\">High boundary of the return value</param>");
            newCode.WriteLine("        /// <returns>return the IQueryable</returns>");
            newCode.WriteLine("        public static IQueryable<TSource> Between<TSource, TKey>");
            newCode.WriteLine("             (this IQueryable<TSource> source,");
            newCode.WriteLine("              Expression<Func<TSource, TKey>> keySelector,");
            newCode.WriteLine("              TKey low, TKey high) where TKey : IComparable<TKey>");
            newCode.WriteLine("        {");
            newCode.WriteLine("            // Get a ParameterExpression node of the TSource that is used in the expression tree");
            newCode.WriteLine("            ParameterExpression sourceParameter = Expression.Parameter(typeof(TSource));");
            newCode.WriteLine("");
            newCode.WriteLine("            // Get the body and parameter of the lambda expression");
            newCode.WriteLine("            Expression body = keySelector.Body;");
            newCode.WriteLine("            ParameterExpression parameter = null;");
            newCode.WriteLine("            if (keySelector.Parameters.Count > 0)");
            newCode.WriteLine("                parameter = keySelector.Parameters[0];");
            newCode.WriteLine("");
            newCode.WriteLine("            // Get the Compare method of the type of the return value");
            newCode.WriteLine("            MethodInfo compareMethod = typeof(TKey).GetMethod(\"CompareTo\", new[] { typeof(TKey) });");
            newCode.WriteLine("");
            newCode.WriteLine("            // Expression.LessThanOrEqual and Expression.GreaterThanOrEqua method are only used in");
            newCode.WriteLine("            // the numeric comparision. If we want to compare the non-numeric type, we can't directly ");
            newCode.WriteLine("            // use the two methods. ");
            newCode.WriteLine("            // So we first use the Compare method to compare the objects, and the Compare method ");
            newCode.WriteLine("            // will return a int number. Then we can use the LessThanOrEqual and GreaterThanOrEqua method.");
            newCode.WriteLine("            // For this reason, we ask all the TKey type implement the IComparable<> interface.");
            newCode.WriteLine("            Expression upper = Expression.LessThanOrEqual(");
            newCode.WriteLine("                Expression.Call(body, compareMethod, Expression.Constant(high)),");
            newCode.WriteLine("                Expression.Constant(0, typeof(int)));");
            newCode.WriteLine("            Expression lower = Expression.GreaterThanOrEqual(");
            newCode.WriteLine("                Expression.Call(body, compareMethod, Expression.Constant(low)),");
            newCode.WriteLine("                Expression.Constant(0, typeof(int)));");
            newCode.WriteLine("");
            newCode.WriteLine("            Expression andExpression = Expression.And(upper, lower);");
            newCode.WriteLine("");
            newCode.WriteLine("            // Get the Where method expression.");
            newCode.WriteLine("            MethodCallExpression whereCallExpression = Expression.Call(");
            newCode.WriteLine("                typeof(Queryable),");
            newCode.WriteLine("                \"Where\",");
            newCode.WriteLine("                new Type[] { source.ElementType },");
            newCode.WriteLine("                source.Expression,");
            newCode.WriteLine("                Expression.Lambda<Func<TSource, bool>>(andExpression,");
            newCode.WriteLine("                new ParameterExpression[] { parameter }));");
            newCode.WriteLine("");
            newCode.WriteLine("            return source.Provider.CreateQuery<TSource>(whereCallExpression);");
            newCode.WriteLine("        }");
            newCode.WriteLine("");
        }
